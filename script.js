...
async function loadData() {
    ...
    } catch (error) { showToast('Не удалось загрузить данные', 'error'); loadLocalData(); }
}

function loadLocalData() {
    const saved = localStorage.getItem('bearTapData');
    if (saved) { playerData = JSON.parse(saved); const timePassed = (Date.now() - playerData.lastSave) / 1000; playerData.energy = Math.min(playerData.maxEnergy || 1000, (playerData.energy || 1000) + Math.floor(timePassed / 2)); }
...
async function saveProgress() {
    ...
    try { await fetch(`${API_URL}/api/save_progress`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, xp: playerData.xp, clicks: playerData.totalClicks, energy: Math.floor(playerData.energy) }) }); lastSavedXp = playerData.xp; } catch (error) {}
}
...
async function buyItem(itemId) {
    const userId = getUserId(); if (!userId) return;
    const item = [{id: 'click_power_2', price: 250000, name: '⚡ Сила клика x2'}, {id: 'click_power_5', price: 1000000, name: '⚡ Сила клика x5'}, {id: 'click_power_10', price: 5000000, name: '⚡⚡ Сила клика x10'}, {id: 'max_energy_2000', price: 500000, name: '🔋 Энергия 2000'}, {id: 'max_energy_5000', price: 2000000, name: '🔋🔋 Энергия 5000'}, {id: 'energy_regen_2', price: 750000, name: '⚡ Реген x2'}, {id: 'energy_regen_5', price: 3000000, name: '⚡⚡ Реген x5'}, {id: 'skin_gold', price: 1000000, name: '🌟 Золотой мишка'}, {id: 'skin_diamond', price: 5000000, name: '💎 Алмазный мишка'}, {id: 'skin_rainbow', price: 10000000, name: '🌈 Радужный мишка'}].find(i => i.id === itemId);
...
function openGame(game) { switchScreen(`game-${game}`); if (game === 'mines') initMinesGrid(); if (game === 'ninja') initNinjaGrid(); }
...
async function startCrash() {
    ...
    try { const response = await fetch(`${API_URL}/api/crash`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'start', bet: bet }) }); const data = await response.json(); if (data.status === 'started') { crashPoint = data.crash_point; const display = document.getElementById('crash-multiplier'); const cashoutBtn = document.getElementById('crash-cashout'); const startBtn = document.getElementById('crash-start'); if (display) { display.textContent = '1.00x'; display.style.color = '#FFD700'; } if (cashoutBtn) cashoutBtn.disabled = false; if (startBtn) startBtn.disabled = true; crashInterval = setInterval(() => { currentMultiplier += 0.01; if (display) display.textContent = currentMultiplier.toFixed(2) + 'x'; if (currentMultiplier >= crashPoint) { clearInterval(crashInterval); if (display) { display.textContent = `💥 CRASH @ ${crashPoint.toFixed(2)}x`; display.style.color = '#ff4757'; } if (cashoutBtn) cashoutBtn.disabled = true; if (startBtn) startBtn.disabled = false; showToast(`💥 Краш на ${crashPoint.toFixed(2)}x!`, 'error'); currentGame = null; loadData(); } }, 100); } else showToast(' ' + (data.error || 'Ошибка'), 'error'); } catch (error) { showToast(' Ошибка', 'error'); }
}

async function cashoutCrash() {
    const userId = getUserId(); if (!userId || currentGame !== 'crash') return; clearInterval(crashInterval);
    try { const response = await fetch(`${API_URL}/api/crash`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'cashout' }) }); const data = await response.json(); if (data.status === 'won') { showToast(`✅ Вы забрали ${data.win} XP!`, 'success'); const display = document.getElementById('crash-multiplier'); const cashoutBtn = document.getElementById('crash-cashout'); const startBtn = document.getElementById('crash-start'); if (display) { display.textContent = `✅ WON @ ${data.multiplier.toFixed(2)}x`; display.style.color = '#2ed573'; } if (cashoutBtn) cashoutBtn.disabled = true; if (startBtn) startBtn.disabled = false; currentGame = null; loadData(); } } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function startMines() {
    ...
    try { const response = await fetch(`${API_URL}/api/mines`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'start', bet: bet, mines: minesCount }) }); const data = await response.json(); if (data.status === 'started') { initMinesGrid(); const cashoutBtn = document.getElementById('mines-cashout'); const startBtn = document.getElementById('mines-start'); if (cashoutBtn) { cashoutBtn.textContent = '💰 Забрать'; cashoutBtn.disabled = false; } if (startBtn) startBtn.disabled = true; updateMinesInfo(); } else showToast('❌ ' + (data.error || 'Ошибка'), 'error'); } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function revealMine(index) {
    ...
    try { const response = await fetch(`${API_URL}/api/mines`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'reveal', cell: index }) }); const data = await response.json(); cell.classList.add('revealed'); if (data.status === 'lost') { cell.classList.add('mine'); cell.textContent = ''; showToast('💥 Вы подорвались на мине!', 'error'); const cashoutBtn = document.getElementById('mines-cashout'); const startBtn = document.getElementById('mines-start'); if (cashoutBtn) { cashoutBtn.textContent = '💰 Забрать'; cashoutBtn.disabled = true; } if (startBtn) startBtn.disabled = false; currentGame = null; minesState = null; loadData(); } else if (data.status === 'safe') { cell.classList.add('safe'); cell.textContent = '💎'; minesState.multiplier = data.multiplier; updateMinesInfo(); const cashoutBtn = document.getElementById('mines-cashout'); if (cashoutBtn) { cashoutBtn.textContent = `💰 Забрать ${data.win} XP`; cashoutBtn.disabled = false; } } } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function cashoutMines() {
    ...
    try { const response = await fetch(`${API_URL}/api/mines`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'cashout' }) }); const data = await response.json(); if (data.status === 'won') { showToast(`✅ Вы забрали ${data.win} XP!`, 'success'); const cashoutBtn = document.getElementById('mines-cashout'); const startBtn = document.getElementById('mines-start'); if (cashoutBtn) { cashoutBtn.textContent = '💰 Забрать'; cashoutBtn.disabled = true; } if (startBtn) startBtn.disabled = false; currentGame = null; minesState = null; loadData(); } } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function spinRoulette(color) {
    ...
    if (!bet || bet < 100 || bet > playerData.xp) { showToast('❌ Неверная ставка!', 'error'); return; }
    try { const response = await fetch(`${API_URL}/api/roulette`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, bet: bet, color: color }) }); const data = await response.json(); const resultEl = document.getElementById('roulette-result'); if (resultEl) { const colorEmojis = {red: '🔴', black: '⚫', green: ''}; const colorNames = {red: 'Красное', black: 'Чёрное', green: 'Зелёное'}; resultEl.textContent = `Выпало: ${colorEmojis[data.result]} ${colorNames[data.result]}`; if (data.status === 'won') { resultEl.textContent += ` | Выигрыш: ${data.win} XP! 🎉`; resultEl.style.color = '#2ed573'; showToast(`✅ Вы выиграли ${data.win} XP!`, 'success'); } else { resultEl.textContent += ' | Вы проиграли 😔'; resultEl.style.color = '#ff4757'; showToast('❌ Вы проиграли', 'error'); } } loadData(); } catch (error) { showToast(' Ошибка', 'error'); }
}
...
async function playDouble(choice) {
    ...
    if (!bet || bet < 100 || bet > playerData.xp) { showToast('❌ Неверная ставка!', 'error'); return; }
    try { const response = await fetch(`${API_URL}/api/double`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, bet: bet, choice: choice }) }); const data = await response.json(); const resultEl = document.getElementById('double-result'); if (resultEl) { const colorEmojis = {red: '', black: '⚫', green: '🟢'}; const colorNames = {red: 'Красное', black: 'Чёрное', green: 'Зелёное'}; resultEl.textContent = `Выпало: ${colorEmojis[data.result]} ${colorNames[data.result]}`; if (data.status === 'won') { resultEl.textContent += ` | Выигрыш: ${data.win} XP! 🎉`; resultEl.style.color = '#2ed573'; showToast(`✅ Вы выиграли ${data.win} XP!`, 'success'); } else { resultEl.textContent += ' | Вы проиграли '; resultEl.style.color = '#ff4757'; showToast('❌ Вы проиграли', 'error'); } } loadData(); } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function startNinja() {
    ...
    try { const response = await fetch(`${API_URL}/api/ninja`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'start', bet: bet }) }); const data = await response.json(); if (data.status === 'started') { initNinjaGrid(); const cashoutBtn = document.getElementById('ninja-cashout'); const startBtn = document.getElementById('ninja-start'); if (cashoutBtn) cashoutBtn.disabled = false; if (startBtn) startBtn.disabled = true; updateNinjaInfo(); } else showToast('❌ ' + (data.error || 'Ошибка'), 'error'); } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function pickNinja(index) {
    ...
    try { const response = await fetch(`${API_URL}/api/ninja`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'pick', pick: index }) }); const data = await response.json(); cell.classList.add('revealed'); if (data.status === 'hit') { cell.classList.add('hit'); cell.textContent = '🥷'; showToast('💥 Вы попали на ниндзя!', 'error'); const cashoutBtn = document.getElementById('ninja-cashout'); const startBtn = document.getElementById('ninja-start'); if (cashoutBtn) cashoutBtn.disabled = true; if (startBtn) startBtn.disabled = false; currentGame = null; ninjaState = null; loadData(); } else if (data.status === 'safe') { cell.classList.add('safe'); cell.textContent = '✅'; ninjaState.multiplier = data.multiplier; ninjaState.rounds = data.rounds; updateNinjaInfo(); } } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function cashoutNinja() {
    ...
    try { const response = await fetch(`${API_URL}/api/ninja`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, action: 'cashout' }) }); const data = await response.json(); if (data.status === 'won') { showToast(`✅ Вы забрали ${data.win} XP!`, 'success'); const cashoutBtn = document.getElementById('ninja-cashout'); const startBtn = document.getElementById('ninja-start'); if (cashoutBtn) cashoutBtn.disabled = true; if (startBtn) startBtn.disabled = false; currentGame = null; ninjaState = null; loadData(); } } catch (error) { showToast('❌ Ошибка', 'error'); }
}
...
async function loadAdminStats() {
    const userId = getUserId(); if (!userId) return;
    try { const response = await fetch(`${API_URL}/api/admin_stats`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ admin_id: userId }) }); const data = await response.json(); if (data.total_users !== undefined) { document.getElementById('admin-total-users').textContent = data.total_users; document.getElementById('admin-active-users').textContent = data.active_users; document.getElementById('admin-banned-users').textContent = data.banned_users; document.getElementById('admin-total-xp').textContent = data.total_xp.toLocaleString(); document.getElementById('admin-avg-xp').textContent = Math.round(data.avg_xp || 0).toLocaleString(); document.getElementById('admin-total-refs').textContent = data.total_refs.toLocaleString(); document.getElementById('admin-total-purchases').textContent = data.total_purchases.toLocaleString(); document.getElementById('admin-total-revenue').textContent = data.total_revenue.toLocaleString(); document.getElementById('admin-total-games').textContent = data.total_games.toLocaleString(); const maintText = document.getElementById('maintenance-text'); const maintBtn = document.getElementById('maintenance-btn'); if (maintText) { maintText.textContent = data.maintenance ? 'Закрыто' : 'Открыто'; maintText.className = data.maintenance ? 'closed' : ''; } if (maintBtn) maintBtn.textContent = data.maintenance ? '🔓 Открыть доступ' : '🔒 Закрыть доступ'; const topList = document.getElementById('admin-top-users'); if (topList && data.top_users && data.top_users.length > 0) { topList.innerHTML = ''; data.top_users.forEach((user, index) => { const rankClass = index === 0 ? 'gold' : index === 1 ? 'silver' : index === 2 ? 'bronze' : ''; const div = document.createElement('div'); div.className = 'top-item'; div.innerHTML = `<div class="top-rank ${rankClass}">${index + 1}</div><div class="top-name">${user.name || user.username || 'User'}</div><div class="top-value">${user.xp.toLocaleString()} XP</div>`; topList.appendChild(div); }); } } } catch (error) { showToast('❌ Ошибка загрузки статистики', 'error'); }
}
...
async function adminBanUser() { const userId = getUserId(); const targetId = document.getElementById('admin-user-id').value; if (!userId || !targetId) { showToast('❌ Введите User ID', 'error'); return; } if (!confirm(` Забанить пользователя ${targetId}?`)) return; try { const response = await fetch(`${API_URL}/api/admin_action`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ admin_id: userId, action: 'ban', target_id: parseInt(targetId) }) }); const data = await response.json(); if (data.status === 'banned') showToast(`✅ Пользователь ${targetId} забанен`, 'success'); else showToast('❌ ' + (data.error || 'Ошибка'), 'error'); } catch (error) { showToast(' Ошибка', 'error'); } }

async function adminUnbanUser() { const userId = getUserId(); const targetId = document.getElementById('admin-user-id').value; if (!userId || !targetId) { showToast(' Введите User ID', 'error'); return; } if (!confirm(`✅ Разбанить пользователя ${targetId}?`)) return; try { const response = await fetch(`${API_URL}/api/admin_action`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ admin_id: userId, action: 'unban', target_id: parseInt(targetId) }) }); const data = await response.json(); if (data.status === 'unbanned') showToast(`✅ Пользователь ${targetId} разбанен`, 'success'); else showToast('❌ ' + (data.error || 'Ошибка'), 'error'); } catch (error) { showToast('❌ Ошибка', 'error'); } }

document.getElementById('bear').addEventListener('click', function(e) { if (playerData.energy < playerData.clickPower) { showToast('⚠️ Недостаточно энергии!', 'error'); if (tg.HapticFeedback) tg.HapticFeedback.notificationOccurred('error'); return; } playerData.xp += playerData.clickPower; playerData.totalClicks++; playerData.energy -= playerData.clickPower; const newLevel = Math.floor(playerData.totalClicks / 1000) + 1; if (newLevel > playerData.level) { playerData.level = newLevel; showToast(`🎉 Новый уровень: ${playerData.level}!`, 'success'); if (tg.HapticFeedback) tg.HapticFeedback.notificationOccurred('success'); } else { if (tg.HapticFeedback) tg.HapticFeedback.impactOccurred('medium'); } showClickEffect(e); updateUI(); updateProfile(); saveData(); });

function showClickEffect(e) { const effect = document.createElement('div'); effect.className = 'click-effect'; effect.textContent = '+' + playerData.clickPower; const rect = e.target.getBoundingClientRect(); const x = e.clientX - rect.left; const y = e.clientY - rect.top; effect.style.left = x + 'px'; effect.style.top = y + 'px'; const bearEl = document.getElementById('bear'); if (bearEl) bearEl.appendChild(effect); setTimeout(() => effect.remove(), 1000); }

function switchScreen(screen) { document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active')); document.querySelectorAll('.screen').forEach(s => s.classList.remove('active')); const navBtn = document.querySelector(`.nav-btn[data-screen="${screen}"]`); if (navBtn) navBtn.classList.add('active'); const targetScreen = document.getElementById(`screen-${screen}`); if (targetScreen) targetScreen.classList.add('active'); if (tg.HapticFeedback) tg.HapticFeedback.selectionChanged(); if (screen === 'profile') updateProfile(); if (screen === 'shop') renderShop(); if (screen === 'wheel') checkWheelTimer(); }

document.querySelectorAll('.nav-btn').forEach(btn => { btn.addEventListener('click', function() { switchScreen(this.dataset.screen); }); });

function showToast(text, type = 'info') { const toast = document.createElement('div'); toast.className = 'toast' + (type === 'error' ? ' error' : ''); toast.textContent = text; document.body.appendChild(toast); toasts.push(toast); updateToastPositions(); setTimeout(() => { toast.style.opacity = '0'; toast.style.transform = 'translateX(-50%) translateY(-20px)'; setTimeout(() => { toast.remove(); toasts = toasts.filter(t => t !== toast); updateToastPositions(); }, 300); }, 2500); }

function updateToastPositions() { const gap = 10; toasts.forEach((toast, index) => { const offset = index * (60 + gap); toast.style.top = `${20 + offset}px`; }); }

setInterval(() => { if (playerData.energy < playerData.maxEnergy) { playerData.energy = Math.min(playerData.maxEnergy, playerData.energy + 1); updateUI(); saveData(); } }, 2000);

setInterval(saveProgress, 10000); setInterval(checkWheelTimer, 1000);

loadData(); showToast('🐻 Тапай и зарабатывай XP!', 'success');
